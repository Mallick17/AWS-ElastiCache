...
The goal is to monitor an **AWS ElastiCache Redis instance** and log its health status.

* **Check frequency**: Every 10 seconds for **5 minutes**.
* **Break**: Pause for 5 minutes.
* **Cycles**: Repeat the above **4 times per run**.
* **Logging**: Maintain separate logs for **success** and **error**.
* **Automation**: Run periodically using **cron**.

---

## 2. Script: `redis_health_cycle.sh`

Create the script in `/usr/local/bin/redis_health_cycle.sh`:
...
for cycle in {1..4}; do
    ...
    echo "=== Cycle $cycle started at $(date "+%Y-%m-%d %H:%M:%S") ===" >> $ERROR_LOG

    # Run for 5 minutes (300s), checking every 10s = ~30 checks
    END=$((SECONDS+300))
    while [ $SECONDS -lt $END ]; do
        ...
        if [ "$RESPONSE" == "PONG" ]; then
            echo "$TIMESTAMP SUCCESS" >> $SUCCESS_LOG
        else
            echo "$TIMESTAMP FAILURE : $RESPONSE" >> $ERROR_LOG
        fi

        sleep 10   # check interval (change to 15 if required)
    done

    echo "=== Cycle $cycle finished, taking 5m break ===" >> $SUCCESS_LOG
    echo "=== Cycle $cycle finished, taking 5m break ===" >> $ERROR_LOG

    # Break for 5 minutes unless it's the last cycle
    if [ $cycle -lt 4 ]; then
        sleep 300
    fi
done

echo "=== All 4 cycles completed at $(date "+%Y-%m-%d %H:%M:%S") ===" >> $SUCCESS_LOG
echo "=== All 4 cycles completed at $(date "+%Y-%m-%d %H:%M:%S") ===" >> $ERROR_LOG
```
...
```bash
chmod +x /usr/local/bin/redis_health_cycle.sh
```
...
* ✅ Success log → `/var/log/redis_success.log`
* ❌ Error log → `/var/log/redis_error.log`

Sample **success log**:

```
=== Cycle 1 started at 2025-08-27 05:40:01 ===
2025-08-27 05:40:01 SUCCESS
2025-08-27 05:40:11 SUCCESS
2025-08-27 05:40:21 SUCCESS
...
=== Cycle 1 finished, taking 5m break ===
```

Sample **error log** (if Redis is unreachable):

```
=== Cycle 1 started at 2025-08-27 05:40:01 ===
2025-08-27 05:40:11 FAILURE : Could not connect to Redis at ...
```
...
```bash
/usr/local/bin/redis_health_cycle.sh
```

Check logs:

```bash
tail -f /var/log/redis_success.log
tail -f /var/log/redis_error.log
```
...
Add this entry to run script every 40 minutes (adjust as needed):

```
*/40 * * * * /usr/local/bin/redis_health_cycle.sh >> /var/log/redis_cron_debug.log 2>&1
```

* `*/40` → Runs every 40 minutes.
* `>> /var/log/redis_cron_debug.log 2>&1` → Captures debug output.
...
1. Ensure **redis-cli path** is correct (`/usr/local/bin/redis-cli`).
...
✔ Script monitors Redis with **10s interval for 5m → 5m break**.
✔ Runs for **4 cycles per execution**.
